...
of ten backends with `/web-tools`, or point Firecrawl, SearXNG, or Ollama at an
instance you run yourself so your queries never leave your network.
...
Most hosted `web_search` providers need credentials. Self-hosted Firecrawl,
SearXNG, and local Ollama can run without a vendor key. Run:

```
/web-tools
```

Pick a provider from the list and enter its API key or instance URL. Sign-up
links for all ten backends are in [Providers](https://github.com/juicesharp/rpiv-mono/blob/main/packages/rpiv-web-tools/docs/providers.md); `brave` is the default. The active
provider is listed first with `✓`, and any provider you have already configured
is marked `(configured)`. Selecting Firecrawl, SearXNG, or Ollama prompts for a
base URL first; each supports an instance you run yourself.
...
- **Nothing has to leave your network** — Firecrawl, SearXNG, and Ollama support
  self-hosted instances with their own base-URL resolution and setup flow.
  Their self-hosted modes do not require a vendor key.
...
| `baseUrls.<provider>` | Instance URL for `firecrawl` / `searxng` / `ollama` | provider default |
...
- [Self-hosted backends](https://github.com/juicesharp/rpiv-mono/blob/main/packages/rpiv-web-tools/docs/self-hosted.md) — Firecrawl, SearXNG, and Ollama setup,
...
- **Credentials for at least one search provider** before `web_search` works.
  ...
  export the env var.` Self-hosted Firecrawl needs a compatible endpoint,
  SearXNG needs a reachable instance with `json` enabled under `search.formats`,
  and Ollama needs a running instance; none requires a vendor key.
