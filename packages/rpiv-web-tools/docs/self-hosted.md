...
Three of the ten providers can talk to an instance you run yourself:
**Firecrawl**, **SearXNG**, and **Ollama**. All three take a base URL in addition
to (or instead of) an API key, and all drive their own prompt flow inside
`/web-tools` — URL first, then the optional key.

## Firecrawl

Firecrawl defaults to Firecrawl Cloud, but any Firecrawl-compatible self-hosted
endpoint can replace it:

```sh
export FIRECRAWL_API_URL=http://localhost:3002/v1
# Optional for a self-hosted endpoint
export FIRECRAWL_API_KEY=…
```

Base-URL resolution is `FIRECRAWL_API_URL` → `baseUrls.firecrawl` in the config
file → `https://api.firecrawl.dev/v1`. `/web-tools` prompts for the URL first
and the key second.

The Cloud default requires `FIRECRAWL_API_KEY` or `apiKeys.firecrawl`.
A non-default self-hosted URL may omit the key; in that case the provider sends
no `Authorization` header. Search requests go to `<base-url>/search` and fetch
requests go to `<base-url>/scrape`.
...
fetched **on the model's behalf**, not to provider endpoints. A `FIRECRAWL_API_URL`, `SEARXNG_URL`, or `OLLAMA_HOST`
pointing at `http://localhost` is intentionally reachable — these providers can
be self-hosted by design. See
